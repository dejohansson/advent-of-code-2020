...
fn main() {
    ...
    let passwords =
        fs::read_to_string("passwords.txt").expect("Something went wrong reading the file");
    ...
    for line in p.iter() {
        ...
        if (val_1 == bounded_char && val_2 != bounded_char)
            || (val_1 != bounded_char && val_2 == bounded_char)
        {
            n_valid_passwords_official += 1;
        }
    }
    println!(
        "Number of valid unofficial passwords: {}",
        n_valid_passwords_unofficial
    );
    println!(
        "Number of valid official passwords: {}",
        n_valid_passwords_official
    );
}
